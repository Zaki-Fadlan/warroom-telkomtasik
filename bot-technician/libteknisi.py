...
def kbOrder_id(chat_id):
  ...
  raise_on_warnings = True
    ...
    cursor = db.cursor()
    sql = "SELECT id_teknisi FROM user_teknisi WHERE id_tele_tek = "+str(chat_id)
    cursor.execute(sql)
    results = cursor.fetchall()
    # ID Teknisi
    id_teknisi =0
    for data in results:
        id_teknisi += int(str(data).replace(',','').replace('(','').replace(')',''))
    sql = "SELECT order_id FROM all_wo WHERE st_wo=8 AND id_nama_teknisi = "+str(id_teknisi)
    cursor.execute(sql)
    results = cursor.fetchall()
    kb=[]
...
def cekIdKendala(chat_id):
  ...
  raise_on_warnings = True
    ...
    cursor = db.cursor()
    sql = "SELECT alamat_inst_tpken_ft FROM user_teknisi_crud_tele WHERE tele_id_ft = "+str(chat_id)
    cursor.execute(sql)
    results = cursor.fetchall()
    for data in results:
...
def cekOrderIdTeknisi(chat_id,order_id):
  ...
  raise_on_warnings = True
    ...
    for data in results:
        id_teknisi += int(str(data).replace(',','').replace('(','').replace(')',''))
    sql = "SELECT order_id FROM all_wo WHERE id_nama_teknisi = "+str(id_teknisi)
    cursor.execute(sql)
    results = cursor.fetchall()
    kb=[]
    for data in results:
...
def read_aksi_teknisi(chat_id):
  ...
  raise_on_warnings = True
    ...
    cursor = db.cursor()
    sql = "SELECT aksi_tele_tek FROM user_teknisi WHERE id_tele_tek = "+str(chat_id)
    cursor.execute(sql)
    results = cursor.fetchall()
    for data in results:
...
def to_crud(chat_id,col_name,value_col):
  ...
  database="db_sisfowtsm"
    ...
    cursor = db.cursor()
    sql = "UPDATE user_teknisi_crud_tele SET "+str(col_name)+"= %s WHERE tele_id_ft=%s"
...
def cek_crud(chat_id):
  ...
  database="db_sisfowtsm"
    ...
    cursor = db.cursor()
    sql = "SELECT tele_id_ft FROM user_teknisi_crud_tele WHERE tele_id_ft = "+str(chat_id)
    cursor.execute(sql)
    results = cursor.fetchall()
    if (len(results)==1):
        return True
...
def cekpengisiandata(chat_id,col_name):
  ...
  database="db_sisfowtsm"
    ...
    cursor = db.cursor()
    sql = "SELECT "+str(col_name)+" FROM user_teknisi_crud_tele WHERE tele_id_ft = "+str(chat_id)
    cursor.execute(sql)
    results = cursor.fetchall()
    for data in results:
        if(list(data)[0]==None):
...
def tambahanDetilWo(order_id):
  ...
  database="db_sisfowtsm"
    ...
    cursor = db.cursor()
    sql ='SELECT track_id,ncp FROM all_wo WHERE order_id = "'+str(order_id)+'"'
    cursor.execute(sql)
    results = cursor.fetchall()
...
def inputSummary(chat_id):
  ...
  database="db_sisfowtsm"
    ...
    cursor = db.cursor()
    sql ="""SELECT * FROM user_teknisi_crud_tele WHERE tele_id_ft = """+str(chat_id)
    cursor.execute(sql)
    results = cursor.fetchall()
    hasil_data = list(results[0])
    ...
    if(hasil_data[17]==3):
        ...
        pesan += "\nNama Cal.Pelanggan : " + str(tambahanDetilWo(str(hasil_data[2]))[1])
        pesan += "\nAlamat Instalasi : " + str(hasil_data[3])
        pesan += "\nODP : " + str(hasil_data[14])
        pesan += "\nContact Person : " + str(hasil_data[4])
        pesan += "\nTikor Pelanggan : " + str(hasil_data[5]).replace("$$$",",")
        ...
        if(int(hasil_data[3])==6 or int(hasil_data[3])==7):
            sql ="""SELECT n_tipe_kendala  FROM dm_kendala WHERE id_kendala = """+str(hasil_data[3])
            cursor.execute(sql)
            results = cursor.fetchall()
...
def pindahData(chat_id):
  ...
  database="db_sisfowtsm"
    ...
    if(status==3):
        sql = "UPDATE all_wo SET alamat_inst='"+str(alm)+"',no_plgn='"+str(ncp)+"',odp='"+str(odp)+"',tikor_plgn='"+str(tikor)+"',port='"+str(port)+"',qr='"+str(qr)+"',pnj_dc='"+str(pjdc)+"',snont='"+str(snont)+"',snstb='"+str(snstb)+"',id_vallins='"+str(idval)+"',user_crew='"+str(usercrew)+"',app_sektor='"+str(appsektor)+"',ket_teknisi='"+str(keterangan)+"',wf_teknisi='"+str(waktu)+"',st_wo='"+str(status)+"' WHERE all_wo.order_id = '"+str(orderid)+"'"
        cursor.execute(sql)
        return db.commit()
    else:
        if(int(alm)==6 or int(alm)==7):
            sql = "UPDATE all_wo SET tp_kendala='"+str(alm)+"',odp='"+str(odp)+"',ket_teknisi='"+str(keterangan)+"',wf_teknisi='"+str(waktu)+"',st_wo='"+str(status)+"' WHERE all_wo.order_id = '"+str(orderid)+"'"
            cursor.execute(sql)
            return db.commit()
        if(int(alm)==4):
            sql = "UPDATE all_wo SET tp_kendala='"+str(alm)+"',no_plgn='"+str(ncp)+"',tikor_plgn='"+str(tikor)+"',manja_date='"+str(port)+"',ket_teknisi='"+str(keterangan)+"',wf_teknisi='"+str(waktu)+"',st_wo='"+str(status)+"' WHERE all_wo.order_id = '"+str(orderid)+"'"
            cursor.execute(sql)
            return db.commit()
        else:
            sql = "UPDATE all_wo SET tp_kendala='"+str(alm)+"',tikor_plgn='"+str(tikor)+"',ket_teknisi='"+str(keterangan)+"',wf_teknisi='"+str(waktu)+"',st_wo='"+str(status)+"' WHERE all_wo.order_id = '"+str(orderid)+"'"
            cursor.execute(sql)
            return db.commit()
...
def listWoKu(chat_id):
  ...
  raise_on_warnings = True
    ...
    for data in results:
        id_teknisi += int(str(data).replace(',','').replace('(','').replace(')',''))
    sql = "SELECT order_id,st_wo FROM all_wo WHERE id_nama_teknisi = "+str(id_teknisi)
    cursor.execute(sql)
    results = cursor.fetchall()
    kb=[]
    # kbmenu = [[KeyboardButton("PROSES WO")],[KeyboardButton("LISTWO")]]
    ...
    for data in kb:
        ...
        for data in results:
            if(no==150):
                return myWo
            myWo+="\n"+str(no)+". "+listwo[0]+" ("+str(data).replace(',','').replace('(','').replace(')','').replace('\'','')+")"
