                      ...
                      <div class="row">
                          <div class="col-sm-3">
                              ...
                              <div class="row">
                                  ...
                                  <script>
                                      function allDataWO() {
                                          $.ajax({
                                              url: "/PvitaMonitor/ajaxDataTableMonitor",
                                              ...
                                              success: function(data) {
                                                  ...
                                                  $('#tabelValidasi').DataTable({
                                                      ajax: '<?php echo ('PvitaMonitor/ajaxDataTableMonitor'); ?>',
                                                      dom: 'Bfrtip',
                                                      buttons: [
                                                          'copyHtml5',
                                                          'excelHtml5',
                          ...
                          <div class="col-md">
                              <div class="row card ">
                                  <div class="col-md-12 ">
                                      <label>Status</label>
                                      <div class="form-group clearfix" id="statusWo">
                                          <div class="row">
                                              <?php foreach ($status as $key => $valStat) { ?>
                                                  <div class="icheck-primary d-inline m-1"><input type="checkbox" class="pilihStatus" id="checkboxPrimary<?= $valStat['id_st_wo'] ?>" value="<?= $valStat['id_st_wo'] ?>"><label for="checkboxPrimary<?= $valStat['id_st_wo'] ?>"><?= $valStat['n_st_wo'] ?></label></div>
                                              <?php } ?>
                                          </div>
                                      </div>
                                  </div>
                      ...
                      <table class="table table-hover table-bordered table-striped w-100" id="tabelValidasi">
                          <thead>
                              <tr>
                                  ...
                                  <th class="text-center align-middle">Kcontact</th>
                                  ...
                                  <th class="text-center align-middle">Validator</th>
                                  <th class="text-center align-middle">Waktu Dispatch</th>
                                  ...
                                  <th class="text-center align-middle">Tipe Kendala</th>
                                  ...
                                  <th class="text-center align-middle">Aktifator</th>
                                  <th class="text-center align-middle">Waktu Aktif</th>
  ...
  <script>
      ...
      function pilihDatel(id) {
          ...
          $.ajax({
              ...
              success: function(response) {
                  ...
                  var datSTO = ""
                  datSTO += '<div class="row"><div class="col-lg-12 mb-2"><button type="button" onclick="selectAllSTO(1)" class="btn btn-block btn-success btn-sm text-light">Pilih Semua</button></div>'
                  datSTO += '<div class="col-lg-12 mb-3"><button type="button" onclick="selectAllSTO(0)" class="btn btn-block btn-danger btn-sm text-light">Batal Pilih Semua</button></div></div>'
      ...
      function filterDataTable() {
          ...
          var checkboxes = document.getElementsByClassName('pilihStatus');
          var stat = "";
          for (var i = 0, n = checkboxes.length; i < n; i++) {
              if (checkboxes[i].checked) {
                  stat += "," + checkboxes[i].value;
              }
          }
          if (stat) stat = stat.substring(1); // selecting STO
          ...
          if (parseDate(data.start_time) == null || parseDate(data.end_time) == null || parseDate(data.start_time) > parseDate(data.end_time)) {
              ...
              toastr.warning('Silahkan pilih Status yang tersedia')
          } else {
              $.ajax({
                  type: "POST",
                  url: "/PvitaMonitor/ajaxDataTableMonitorFilter",
                  data: data,
                  cache: false,
                  beforeSend: function(f) {
                      toastr.info('Mencoba mengambil data!!')
