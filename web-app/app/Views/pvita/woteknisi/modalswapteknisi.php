<?php foreach ($data as $key => $valDat) { ?>
    <div class="modal-header">
        <h4 class="modal-title">Ganti Teknisi</h4>
    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control sektor" name="sektor">
                    <?php foreach ($sto as $kSto => $valSto) {
                        if ($valDat['sto'] == $valSto['id_sto']) { ?>
                            <option value="<?= $valDat['id_sto'] ?>" selected="selected"><?= $valSto['nama_sto'] ?></option>
                        <?php }
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            if (response == "\"200\"") {
                                Toast.fire({
                                    icon: 'success',
                                    title: 'Teknisi berhasil diganti'
                                })
                            } else {
                                Toast.fire({
                                    icon: 'warning',
