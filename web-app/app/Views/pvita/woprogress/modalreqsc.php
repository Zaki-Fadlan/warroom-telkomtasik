<?php foreach ($data as $key => $valDat) { ?>
    <div class="modal-header">
        <h4 class="modal-title">Request SC</h4>
    </div>
    <form id="formSTO" method="POST">
        <div class="modal-body">
            <p>Apakah anda yakin Request SC untuk WO <?= $valDat['order_id']; ?> Data ini?</p>
        </div>
        <div class="modal-footer">
            <div class="row">
                <button type="button" class="btn btn-info " onclick="simpan()"><b>Request SC</b></button>
            </div>
        </div>
    </form>
    <script>
        ...
        function simpan() {
            ...
            ?>
                ...
                $.ajax({
                    type: "POST",
                    url: "/PvitaProgress2/updateReqSC",
                    data: data,
                    cache: false,
